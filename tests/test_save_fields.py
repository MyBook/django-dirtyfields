...
from .models import TestModel, TestMixedFieldsModel, TestModelWithForeignKey, TestDeferedModel
...
@pytest.mark.django_db
def test_save_defered_model():
    TestDeferedModel.objects.create()

    tdm = TestDeferedModel.objects.first()
    tdm.boolean = False
...
@pytest.mark.django_db
def test_save_defered_model_with_update_fields():
    TestDeferedModel.objects.create()

    tdm = TestDeferedModel.objects.first()
    tdm.boolean = False

    tdm.save(update_fields=['boolean'])
